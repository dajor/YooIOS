...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 0) { // picture
        ...
        if([members count] > 0){
            @try{
//                BOOL isOnline = [[ChatTools sharedInstance] isPresent:(YooUser *)self.recipient];
//                if(isOnline){
//                    [[ChatTools sharedInstance] requestCall:members listener:self];
//                }else{
//                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:@"User is offline." delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
//                    [alert show];
//                }
                //[self didCallSendOutMessage];
                [[ChatTools sharedInstance] requestCall:members listener:self];
            }
