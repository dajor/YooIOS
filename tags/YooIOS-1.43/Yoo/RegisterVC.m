...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    return self.step == REGISTER_CODE ? 54 : 40;
}
...
- (void)didSelect:(NSArray *)values tag:(NSInteger)tag {
    self.countryCode = [values objectAtIndex:0];
    [self.tableView reloadSections:[NSIndexSet indexSetWithIndex:0] withRowAnimation:UITableViewRowAnimationNone];
}
...
- (void)buildTitle {
    ...
    self.titleLbl.frame = CGRectMake(0, 0, self.titleView.frame.size.width, self.titleView.frame.size.height - 15);
    [self updateHeader];
    ...
    [self.stepView setFrame:CGRectMake((self.titleView.frame.size.width-30)/2, self.titleView.frame.size.height - 14, 30, 10)];
    ...
    for (int i = 0; i < 3; i++) {
        ...
        UIView *stepView = [[UIView alloc] initWithFrame:CGRectMake(i * 10, 1, 8, 8)];
