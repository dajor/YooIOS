...
- (void)createFakeData {
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        NSString *number = [NSString stringWithFormat:@"+491512750%d%d%d%d", i%10, i/10, (i*7)%10, (i*i)%10];
        ABMutableMultiValueRef multiPhone = ABMultiValueCreateMutable(kABMultiStringPropertyType);
        ABMultiValueAddValueAndLabel(multiPhone, (__bridge CFTypeRef)number, (__bridge CFTypeRef)@"home", NULL);
    ...
    if (addressBookOK) {
        ...
        for (Contact *contact in [ContactDAO list]) {
            ...
            if (![createdIds containsObject:cid]) {
                // re-check existing, as the contact may have been created in the meantime
                Contact *exist = [self find:contact.contactId];
                if (exist == nil) {
                    [ContactDAO remove:contact.contactId];
                    for (YooUser *yooUser in [UserDAO list]) {
                        if (yooUser.contactId == contact.contactId) {
                            yooUser.contactId = -1;
                            [UserDAO remove:yooUser.toJID];
                            [ChatDAO deleteForRecipient:yooUser.toJID]; // clear chat
                            if (![self.removed containsObject:yooUser.toJID]) {
                                [self.removed addObject:yooUser.toJID];
                            }
                        }
                    }
                }
