...
- (void)hideIndicator:(NSString *)msgId {
    if (self.isViewLoaded && self.view.window) {
        YooMessage *msg =  [self getMessageById:[msgId integerValue]];
        if (msg) {
            [[ChatTools sharedInstance] cancelCall:msg];
        }
...
- (BOOL)removeBubble:(NSString *)yooId {
    if (yooId != nil) {
...
- (void)didReceiveMessage:(YooMessage *)message {
    ...
    if (message.type !=ymtMessageRead && [message.from.toJID isEqualToString:self.recipient.toJID]) {
        ...
        self.messageCount = CHAT_START_SIZE;
        BOOL existing = [self removeBubble:message.yooId];
        [self update];
        if (!existing) {
            [self scrollDown:YES];
        }
...
- (void)postItem:(id)sender {
    UIButton *button = (UIButton *)sender;
    [self.view endEditing:YES]; //hide keyboard when show actionsheet
...
- (void)startPhoneCall {
    //disable button for 2 second to avoid user press to many time
    self.secRightBtn.enabled = NO;
...
- (void)handlePhoneCall:(YooMessage *)call {
    BOOL existing = [self removeBubble:call.yooId];
    [self update];
    if (!existing) {
        [self scrollDown:YES];
    }
