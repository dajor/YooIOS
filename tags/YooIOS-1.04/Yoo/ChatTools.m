...
- (void)requestCall:(NSArray *)pMembers listener:(NSObject<CallingListener> *)listener {
    ...
    for(YooUser *user in pMembers){
        ...
        names = [self toJSonStringWithKeysAndValues:@"Name", user.toJID, @"Country", [NSString stringWithFormat:@"%d", user.countryCode], nil];
    }
...
- (NSString *) toJSonStringWithKeyAndValueObject:(NSString *) key valueObject:(NSString *)valueObject keysAndValues:(NSString *) first, ... NS_REQUIRES_NIL_TERMINATION{
    ...
    while((arg = va_arg(args, id))){
        if(![arg isEqual:nil] && ![arg isEqual:[NSNull null]] && ![arg isEqualToString:@""]){
            int moduloe = index % 2;
    ...
    if (elements.count > 0) {
        ...
        if ([ns.stringValue isEqualToString:@"jabber:iq:roster"]) {
            ...
            for (int i = 0; i<[userElements count]; i++) {
                ...
                */
                DDXMLElement *numberCountryCodeElt = [[userElt elementsForName:@"country"] objectAtIndex:0];
                NSString *numberCountryCode = numberCountryCodeElt.stringValue;
                ...
                if (contact != nil) {
                    yooUser.contactId = contact.contactId;
                    yooUser.countryCode = [numberCountryCode intValue];
                    [UserDAO upsert:yooUser];
            ...
            if(self.callingDelegate != nil){
                ...
                NSString *senderConferenceNumber = [result objectForKey:@"ConfCallNumber"];
                [self.mappingConferenceNumbers setObject:senderConferenceNumber forKey:login];
        ...
        /*
        else if ([ns.stringValue isEqualToString:@"yoo:iq:callstatus"]){
            if([queryElement.stringValue isEqualToString:@"approval"]){
                NSUserDefaults *userdefault = [NSUserDefaults standardUserDefaults];
                NSString *login = [userdefault stringForKey:@"login"];
                NSString *conferenceNumber = [self.mappingConferenceNumbers objectForKey:login];
                [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[NSString stringWithFormat:@"tel:%@", conferenceNumber]]];
            }else {
                NSLog(@"%@", queryElement.stringValue);
            }
            return YES;
        }*/
    }
