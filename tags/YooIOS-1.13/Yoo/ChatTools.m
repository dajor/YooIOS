...
- (void)sendPending {
    ...
    //unsent messages
    if (![self.login isEqualToString:@"registration"]) {
        ...
        if ([ns.stringValue isEqualToString:@"jabber:iq:roster"]) {
            ...
            for (int i = 0; i < userElts.count; i++) {
                ...
                if (userJid.length > 0 && confNumber.length > 0) {
                    [self.mappingConferenceNumbers setObject:confNumber forKey:userJid];
                }
            }
            NSString *userJid = [NSString stringWithFormat:@"%@@%@", self.login, YOO_DOMAIN];
            NSString *conf = [self.mappingConferenceNumbers objectForKey:userJid];
            BOOL ok = (conf != nil);
            ...
            if (!ok) {
                ...
                [self broadcast:@selector(didCallSendOutMessage) param:nil];
            }
        }
    }
...
//    yooMsg.picture = myData;
    ...
    if(yooMsg.type == ymtCallRequest){
        yooMsg.conferenceNumber = [self.mappingConferenceNumbers objectForKey:[NSString stringWithFormat:@"%@@%@", self.login, YOO_DOMAIN]];
