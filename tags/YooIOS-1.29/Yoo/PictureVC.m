...
- (id)initWithPictures:(NSArray *)pPictures
{
    ...
    if (self) {
        ...
        [self.rightBtn setTitle:NSLocalizedString(@"SAVE_IMAGE", nil) forState:UIControlStateNormal];
        [self.rightBtn addTarget:self action:@selector(saveImage:) forControlEvents:UIControlEventTouchUpInside];
        [self.rightBtn setAutoresizingMask:UIViewAutoresizingFlexibleRightMargin|UIViewAutoresizingFlexibleBottomMargin];
        [self.rightBtn setFrame:CGRectMake(self.view.frame.size.width -50, 0, 44, 44)];
