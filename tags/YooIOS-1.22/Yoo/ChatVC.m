...
- (UIView *)buildBubble:(YooMessage *)yooMsg constraint:(CGSize)constraintSize leftMargin:(CGFloat)leftMargin {
    ...
    if ([yooMsg.from isMe] && yooMsg.type != ymtCallRequest) {
    ...
    if (yooMsg.type == ymtCallRequest) {
        if ((seconds > CALL_MAX_DELAY && yooMsg.callStatus != csAccepted) || yooMsg.callStatus == csRejected || yooMsg.callStatus == csCancelled) {
            UILabel *cancel = [[UILabel alloc] initWithFrame:CGRectMake(bubble.frame.size.width - BUBBLE_MARGIN - 20, BUBBLE_MARGIN, 20, 20)];
            ...
            UILabel *accept = [[UILabel alloc] initWithFrame:CGRectMake(bubble.frame.size.width - BUBBLE_MARGIN - 20, BUBBLE_MARGIN, 20, 20)];
            ...
            [ind setFrame:CGRectMake(bubble.frame.size.width - BUBBLE_MARGIN - 20, BUBBLE_MARGIN, 20, 20)];
            ...
            // cancel button
            if (![yooMsg.to isMe]) {
                UIButton *cancel = [[UIButton alloc] initWithFrame:CGRectMake(BUBBLE_MARGIN * 3, bubble.frame.size.height - BUBBLE_MARGIN - 44, bubble.frame.size.width - BUBBLE_MARGIN * 6, 36)];
...
- (void)update {
    ...
    for (YooMessage *yooMsg in self.messages) {
        ...
        if (bubble == nil) {
            bubble = [self buildBubble:yooMsg constraint:constraintSize leftMargin:leftMargin];
        }
