...
- (void)sendPending {
    ...
    if (![self.login isEqualToString:REGISTRATION_USER]) {
        NSString* name = [NSString stringWithFormat:@"%@@%@",self.login,YOO_DOMAIN];
        NSObject<YooRecipient> * recipient = [UserDAO findByJid:name];
        NSArray *unsentMessage = [ChatDAO unsentList:recipient];
...
- (void)mustSend:(DDXMLElement *)element {
    if (self.xmppStream != nil && [self.xmppStream isConnected] && [self.xmppStream isAuthenticated]) {
        ...
        if ([element.name isEqualToString:@"message"]) {
            ...
            if (msgId.length > 0) {
                [ChatDAO markAsSent:[NSString stringWithFormat:@"%@@%@", self.login, YOO_DOMAIN] ident:msgId];
            }
        }
    } else {
