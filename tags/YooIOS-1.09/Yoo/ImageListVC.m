...
- (id)initWithListener:(NSObject<ImageListListener> *)pListener
{
    ...
    if (self) {
        ...
        [self.navigationItem setRightBarButtonItem:[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"CHAT_SEND", nil) style:UIBarButtonItemStylePlain target:self action:@selector(send)]];
    }
    return self;
}
...
- (void)buildView {
    ...
    [self.deleteBtn setBackgroundColor:[UITools blueColor]];
    ...
    [self.rotateBtn setBackgroundColor:[UITools blueColor]];
    ...
    [self.cropBtn setBackgroundColor:[UITools blueColor]];
...
- (void)updateThumbs {
    ...
    for (NSInteger i = 0; i < MIN(self.images.count+1, 6); i++) {
        ...
        thumbImg.layer.borderColor = (i == self.current) ? [UITools blueColor].CGColor : thumbImg.backgroundColor.CGColor;
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    image = [ImageTools resize:image maxWidth:1024];
