...
+ (void)upsert:(YooUser *)user {
    ...
    if (user.countryCode != nil){
        [item setObject:user.countryCode forKey:@"countryCode"];
    }
...
+ (YooUser *)findByCriteria:(NSObject <Criteria> *)criteria {
    Database *database = [Database getInstance];
    NSArray *existing = [database select:@"yoouser" fields:@[@"jid", @"alias", @"callingCode", @"countryCode", @"contactid", @"picture", @"lastonline"] criteria:criteria limit:1 order:nil];
    if (existing.count > 0) {
...
+ (YooUser *)mapRow:(NSDictionary *)row {
    ...
    user.countryCode = [row objectForKey:@"countryCode"];
...
+ (NSArray *)list {
    ...
    Database *database = [Database getInstance];
    NSArray *rows = [database select:@"yoouser" fields:@[@"jid", @"alias", @"contactid", @"callingCode", @"countryCode", @"picture", @"lastonline"] criteria:nil limit:0 order:nil];
