...
- (void)update {
    ...
    for (YooMessage *yooMsg in self.messages) {
        ...
        if (yooMsg.type == ymtCallRequest) {
            ...
            if ((seconds > 180 && ![yooMsg.message isEqualToString:@"accepted"]) || [yooMsg.message isEqualToString:@"rejected"]) {
            ...
            } else if ([yooMsg.message isEqualToString:@"accepted"]) {
                ...
                [ind startAnimating];
                [self performSelector:@selector(update) withObject:nil afterDelay:seconds];
            }
        } else {
