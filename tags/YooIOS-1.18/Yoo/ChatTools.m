...
- (void)sendPending {
    ...
    if (![self.login isEqualToString:REGISTRATION_USER]) {
        NSObject<YooRecipient> *me = [UserDAO findByJid:[self getMeJid]];
...
- (void)mustSend:(DDXMLElement *)element {
    if (self.xmppStream != nil && [self.xmppStream isConnected] && [self.xmppStream isAuthenticated]) {
        ...
        if ([element.name isEqualToString:@"message"]) {
            ...
            if (msgId.length > 0) {
                [ChatDAO markAsSent:[self getMeJid] ident:msgId];
            }
        }
    } else {
...
- (void)xmppStreamDidAuthenticate:(XMPPStream *)sender {
    ...
    if (![self.login isEqualToString:REGISTRATION_USER]) {
        ...
        if (me.picture == nil) {
            [self requestVCard:[self getMeJid]];
        }
    }
...
- (void)xmppStream:(XMPPStream *)sender didReceiveMessage:(XMPPMessage *)message {
    ...
    if (yooMsg.type == ymtCallStatus) {
        ...
        if (parts.count == 2) {
            NSString *userJid = [NSString stringWithFormat:@"%@@%@", self.login, YOO_DOMAIN];
            BOOL accepted = [parts[0] isEqualToString:@"accepted"];
            [ChatDAO updateCall:userJid ident:parts[1] accepted:accepted];
            if (accepted) {
                NSString *confNumber = [self.conferenceNumbers objectForKey:userJid];
                CallInfo *info = [[CallInfo alloc] initWithStep:pcCallAccepted number:confNumber];
                [self broadcast:@selector(handlePhoneCall:) param:info];
            } else {
                CallInfo *info = [[CallInfo alloc] initWithStep:pcCallRejected number:nil];
                [self broadcast:@selector(handlePhoneCall:) param:info];
    ...
    if (yooMsg.type == ymtCallRequest) {
        ...
        CallInfo *info = [[CallInfo alloc] initWithStep:pcCallRequest number:yooMsg.conferenceNumber];
        info.caller = [yooMsg.from toJID];
        info.callMsgId = yooMsg.ident;
        [self showCallPopup:info];
//        [self broadcast:@selector(handlePhoneCall:) param:info];
    }
...
- (void)showCallPopup:(CallInfo *)info {
    YooUser *user = [UserDAO findByJid:info.caller];
    UICustomAlertViewVC *alert = [[UICustomAlertViewVC alloc] initWithTitle:@"Phone call" message:[NSString stringWithFormat:@"%@ calling!", user.alias] delegate:self cancelButtonTitle:@"Decline" otherButtonTitles:@"Accept", nil];
    alert.callInfo = info;
    [alert show];
...
- (void)requestVCard:(NSString *)jid {
    ...
    */
- (void)requestCall:(NSArray *)pMembers {
...
- (BOOL)xmppStream:(XMPPStream *)sender didReceiveIQ:(XMPPIQ *)iq {
    ...
    if (elements.count > 0) {
        ...
        if ([ns.stringValue isEqualToString:@"jabber:iq:roster"]) {
             ...
             */
            self.conferenceNumbers = [[NSMutableDictionary alloc] init];
            ...
            for (int i = 0; i < userElts.count; i++) {
                ...
                if (userJid.length > 0 && confNumber.length > 0) {
                    [self.conferenceNumbers setObject:confNumber forKey:userJid];
                }
            }
            NSString *userJid = [NSString stringWithFormat:@"%@@%@", self.login, YOO_DOMAIN];
            NSString *conf = [self.conferenceNumbers objectForKey:userJid];
            BOOL ok = (conf != nil);
            ...
            if (!ok) {
                ...
                CallInfo *info = [[CallInfo alloc] initWithStep:pcReceivedConferenceNumber number:nil];
                [self broadcast:@selector(handlePhoneCall:) param:info];
            }
        }
    }
...
//    yooMsg.picture = myData;
    ...
    NSDateFormatter *df = [[NSDateFormatter alloc] init];
    [df setDateFormat:@"yyyy-MM-dd'T'HH:mm:ss:SSS"];
    [self addProperty:@"sendDate" value:[df stringFromDate:[NSDate date]] element:propsElt];
    yooMsg.date = [NSDate date];
    
    if(yooMsg.type == ymtCallRequest){
        yooMsg.conferenceNumber = [self.conferenceNumbers objectForKey:[NSString stringWithFormat:@"%@@%@", self.login, YOO_DOMAIN]];
        NSString *receiverConferenceNumber = [self.conferenceNumbers objectForKey:yooMsg.to.toJID];
...
- (void)xmppStream:(XMPPStream *)sender didReceivePresence:(XMPPPresence *)presence {
    ...
    } else if (![presence.from.user isEqualToString:REGISTRATION_USER] && [presence.from.domain isEqualToString:YOO_DOMAIN]) {
...
- (void)obtainContact:(NSArray *)contactIds {
    ...
    for (NSNumber *contactId in contactIds) {
        ...
        if (fullContact.phones.count > 0) {
            ...
            [idElt setStringValue:[NSString stringWithFormat:@"%ld", contactId.integerValue]];
...
- (void)getUsersFromContacts {
    ...
    for (Contact *contact in contacts) {
        ...
        if (contact.hasPhone) {
            ...
            if (batch.count == 10) {
                [self performSelectorInBackground:@selector(obtainContact:) withObject:[NSArray arrayWithArray:batch]];
                [batch removeAllObjects];
            }
        }
    }
    if (batch.count > 0) {
        [self performSelectorInBackground:@selector(obtainContact:) withObject:batch];
    }
...
//    </iq>
    ...
    NSXMLElement *iqStanza = [NSXMLElement elementWithName: @"iq"];
    [iqStanza addAttribute:[DDXMLNode attributeWithName:@"type" stringValue:@"set"]];
    [iqStanza addAttribute:[DDXMLNode attributeWithName:@"from" stringValue:[self getMeJid]]];
    [iqStanza addAttribute:[DDXMLNode attributeWithName:@"to" stringValue:groupJid]];
...
- (void)answerCall:(CallInfo *)call accept:(BOOL)accept {
    YooMessage *response = [[YooMessage alloc] init];
    response.to = [UserDAO findByJid:call.caller];
    response.type = ymtCallStatus;

    if (accept) {
        // Accept
        response.message = [NSString stringWithFormat:@"accepted:%@", call.callMsgId];
        NSString *telNumber = call.confNumber;
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[NSString stringWithFormat:@"tel:%@", telNumber]]];
    } else {
        // Reject
        response.message = [NSString stringWithFormat:@"rejected:%@", call.callMsgId];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    [[ChatTools sharedInstance] answerCall:alert.callInfo accept:buttonIndex == 1];
}
