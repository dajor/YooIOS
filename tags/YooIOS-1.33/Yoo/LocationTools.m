...
//             DDLogError(@"Geocode failed with error: %@", error);
         ...
         if(self.gpsCountry.length == 0) self.gpsCountry = myPlacemark.ISOcountryCode;
...
- (NSString *)getCountryCode {
    ...
    NSLog(@"current country : %@",self.langCountry);
...
- (NSArray *)fullPhones:(NSString *)phone {
    ...
    if ([phone hasPrefix:@"+"]) {
        NBPhoneNumber *parsedPhone = [phoneUtil parseWithPhoneCarrierRegion:phone error:&aError];
        if (parsedPhone != nil) {
            [myNumbers addObject:parsedPhone];
        }
    } else {
