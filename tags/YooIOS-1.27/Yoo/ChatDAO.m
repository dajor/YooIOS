...
+ (void)initTable {
    ...
    [columns setObject:@"TEXT" forKey:@"readbyother"];
...
+ (void)updateReadMesage:(NSString *)jid ident:(NSString *)ident{
    [msgCache removeObjectForKey:jid];
    Database *database = [Database getInstance];
    NSMutableDictionary *item = [NSMutableDictionary dictionary];
    [item setObject:@"1" forKey:@"readbyother"];
...
+ (void)updateCall:(NSString *)jid ident:(NSString *)ident status:(CallStatus)status {
    ...
    Database *database = [Database getInstance];
    NSMutableDictionary *item = [NSMutableDictionary dictionary];
    [item setObject:[NSString stringWithFormat:@"%ld", (long)status] forKey:@"callstatus"];
    [database update:@"chat" item:item criteria:[[EqualsCriteria alloc] initWithField:@"id" value:ident]];
}
...
+(void)markAsReadByOther:(NSString *)jid ident:(NSString *)ident{
    [msgCache removeObjectForKey:jid];
    Database *database = [Database getInstance];
    NSMutableDictionary *item = [NSMutableDictionary dictionary];
    [item setObject:@"1" forKey:@"readbyother"];
...
+ (NSArray *)unreadList:(NSObject<YooRecipient> *)recipient {
    ...
    [andCrit.criterias addObject:unreadByotherCrit];
...
+ (NSArray *)allFields {
    return @[@"yooid", @"id", @"type", @"message", @"sender", @"recipient", @"read",  @"location", @"ack", @"sent",@"readbyother", @"receipt", @"date", @"shared", @"groupmember", @"sound", @"conferenceNumber", @"callstatus", @"callreqid"];
}
...
+ (YooMessage *)mapRow:(NSDictionary *)row {
    ...
    yooMsg.readByOther = [[row objectForKey:@"readbyother"] isEqualToString:@"1"];
    ...
    if ([[row objectForKey:@"sender"] hasSuffix:CONFERENCE_DOMAIN]) {
        ...
        // this point it will reference object group from cach
        YooGroup *tmpgroup = [GroupDAO find:groupCode];
        YooGroup *group = [[YooGroup alloc]initWithName:tmpgroup.name alias:tmpgroup.alias];
        group.date = tmpgroup.date;
