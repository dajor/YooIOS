...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *section = [self.activity.sections objectAtIndex:indexPath.section];
    NSArray *sectionRcpt = [self.activity.yooRecipients objectForKey:section];
    ...
    if ([recipient isKindOfClass:[YooUser class]]) {
        ...
        contact = [self.activity.contactMap objectForKey:[NSString stringWithFormat:@"%ld", (long)yooUser.contactId]];
    ...
    UIView *rightView = [[UIView alloc]initWithFrame:CGRectMake(10, 0,44, 44)];
    UIButton *call = [UIButton buttonWithType:UIButtonTypeCustom];
    UIImageView *imgBt = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"phone-48.png"]];
    imgBt.frame= CGRectMake(0, 0, 44, 44);
    [call addSubview:imgBt];
    [call addTarget:self action:@selector(startPhoneCall:) forControlEvents:UIControlEventTouchUpInside];
    [call setFrame:CGRectMake(0, 0, 44, 44)];
    [rightView addSubview:call];
    ...
    if (cpt > 0) {
        ...
        UILabel *msgLbl = [[UILabel alloc] initWithFrame:CGRectMake(-(size.width + 16) , 5, size.width + 16, 22)];
        ...
        [rightView addSubview:msgLbl];
    }
    
    call.tag = indexPath.section;
    
    [cell setAccessoryView:rightView];
...
- (void)startPhoneCall:(UIButton *)sender {
    NSString *section = [self.activity.sections objectAtIndex:sender.tag];
    NSArray *sectionRcpt = [self.activity.yooRecipients objectForKey:section];
    NSArray *tuple = [sectionRcpt objectAtIndex:0];
    NSObject<YooRecipient> *recipient = [tuple objectAtIndex:0];
   [[ChatTools sharedInstance] requestCall:recipient];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    return self.activity.sections.count;
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    NSString *sectionName = [self.activity.sections objectAtIndex:section];
    NSArray *sectionUsers = [self.activity.yooRecipients objectForKey:sectionName];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *section = [self.activity.sections objectAtIndex:indexPath.section];
    NSArray *sectionRecpt = [self.activity.yooRecipients objectForKey:section];
...
- (void)handlePhoneCall:(YooMessage *)info {
}
