...
-(void)processMessage:(XMPPMessage *)message{
    ...
    if (yooMsg.type == ymtCallStatus) {
            ...
            if (reqMsg.callStatus != csCancelled) {
                ...
                [self broadcast:@selector(handlePhoneCall:) param:reqMsg];
    ...
    if (yooMsg.type == ymtCallRequest) {
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
            ...
            NSLog(@"recieve callID = %@",yooMsg.ident);
            //in case cancel request was received just before call request message
            ...
            if (cancelled) {
                ...
                YooMessage *reqMsg = [ChatDAO findById:cancelled.callReqId];
                if (reqMsg) {
                    NSString *userJid = [self getMyJid];
                    [ChatDAO updateCall:userJid ident:cancelled.callReqId status:cancelled.callStatus];
...
- (void)answerCall:(YooMessage *)call accept:(BOOL)accept {
    ...
    if ([call.from isKindOfClass:[YooGroup class]]) {
        response.to = [GroupDAO find:((YooGroup *)call.from).name];
    } else {
    ...
    // don't send response for groups - not needed
    if (![call.from isKindOfClass:[YooGroup class]]) {
