...
- (id)init {
    ...
    self.fbId = nil;
    self.fbPicture = nil;
    self.settings = @[@[@"USER_NICKNAME", @"USER_STATUS", @"HIDE_STATUS"], @[@"APP_BACKGROUND"], @[@"STATS_MESSAGES", @"STATS_NETWORK"]];
    ...
    if ([userDefaults objectForKey:@"hideStatus"]==NULL)
    [userDefaults setValue:@"0" forKey:@"hideStatus"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([txt isEqualToString:@"USER_NICKNAME"]) {
        ...
        UInt64 sent = [[ChatTools sharedInstance] stats:YES];
    ...
    } else if ([txt isEqualToString:@"HIDE_STATUS"]) {
        for (UIView *view in [cell.contentView subviews]) {
            if ([view isKindOfClass:[UISwitch class]])[view removeFromSuperview];
        }
        NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
        UISwitch *switchBtn = [[UISwitch alloc] initWithFrame:CGRectMake(120, 8, 64, 48)];
        [switchBtn addTarget:self action:@selector(switchChanged:) forControlEvents:UIControlEventValueChanged];
        switchBtn.on = [[userDefaults objectForKey:@"hideStatus"] boolValue];
...
-(void)switchChanged:(UISwitch *)sender{
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    [userDefaults setObject:[NSString stringWithFormat:@"%i",sender.on] forKey:@"hideStatus"];
    [[ChatTools sharedInstance] sendPresence];
}
...
- (void)handlePhoneCall:(YooMessage *)status {
}
