...
+ (Activity *)getActivity {
    ...
    for (YooUser *yooUser in [UserDAO list]) {
        if (![yooUser isMe] && yooUser.contactId != -1) {
            ...
            if (contact != nil) {
                NSArray *userMsgs = [[ChatTools sharedInstance] messagesForRecipient:yooUser withPicture:NO limit:1];
                if ([userMsgs count] > 0) {
    ...
    for (YooGroup *yooGroup in [GroupDAO list]) {
        ...
        NSArray *groupMsgs = [[ChatTools sharedInstance] messagesForRecipient:yooGroup withPicture:NO limit:1];
        YooMessage *last = nil;
