...
+ (YooMessage *)acknowledge:(NSString *)idMsg {
    ...
    YooMessage *yooMsg = nil;
    if (rows.count > 0) {
        NSDictionary *row = [rows objectAtIndex:0];
        yooMsg = [ChatDAO mapRow:row];
...
+ (void)updateCall:(NSString *)jid ident:(NSString *)ident accepted:(BOOL)accepted {
    ...
    Database *database = [Database getInstance];
    NSObject<Criteria> *idCrit = [[EqualsCriteria alloc] initWithField:@"id" value:ident];
    NSArray *rows = [database select:@"chat" fields:@[@"message"] criteria:idCrit limit:1 order:nil];
    if (rows.count > 0) {
        NSDictionary *row = [rows objectAtIndex:0];
        NSString *message = [NSString stringWithFormat:@"%@ %@", [row objectForKey:@"message"], accepted ? @"accepted" : @"rejected"];
        NSMutableDictionary *item = [NSMutableDictionary dictionary];
...
+ (NSArray *)allFields {
    return @[@"yooid", @"id", @"type", @"message", @"sender", @"recipient", @"read",  @"location", @"ack", @"sent", @"receipt", @"date", @"shared", @"groupmember", @"sound", @"conferenceNumber"];
}
...
+ (YooMessage *)mapRow:(NSDictionary *)row {
    ...
    if ([[row objectForKey:@"sender"] hasSuffix:CONFERENCE_DOMAIN]) {
        ...
        YooGroup *group = [[YooGroup alloc] initWithName:groupCode alias:groupCode];
    ...
    if ([[row objectForKey:@"recipient"] hasSuffix:CONFERENCE_DOMAIN]) {
        ...
        NSString *groupCode = [parts objectAtIndex:0];
        yooMsg.to = [[YooGroup alloc] initWithName:groupCode alias:groupCode];
    } else {
...
+ (NSArray *)list:(NSObject<YooRecipient> *)recipient withPictures:(BOOL)pict {
    ...
    NSArray *rows = [database select:@"chat" fields:[ChatDAO allFields] criteria:orCrit limit:MAX_USER_HISTORY order:@"date DESC"];
    for (NSDictionary *row in rows) {
...
+(NSArray*) unsentList: (NSObject<YooRecipient> *)recipient
{
    ...
    NSArray *yooMessages = [[ChatTools sharedInstance] messagesForRecipient:recipient withPicture:YES];
